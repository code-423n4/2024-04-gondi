...
contract UserVault is ERC721, ERC721TokenReceiver, IUserVault, Owned {
    ...
    mapping(address => mapping(uint256 => uint256)) _vaultOldERC721s;
    ...
    /// @inheritdoc IUserVault
    function depositOldERC721(uint256 _vaultId, address _collection, uint256 _tokenId) external vaultExists(_vaultId) {
        if (!_collectionManager.isWhitelisted(_collection)) {
            revert CollectionNotWhitelistedError();
        }
    ...
    /// @inheritdoc IUserVault
    function depositOldERC721s(uint256 _vaultId, address _collection, uint256[] calldata _tokenIds)
        external
        vaultExists(_vaultId)
    {
        if (!_collectionManager.isWhitelisted(_collection)) {
    ...
    function _withdrawOldERC721(uint256 _vaultId, address _collection, uint256 _tokenId)
        private
        onlyReadyForWithdrawal(_vaultId)
    {
