...
contract MultiSourceLoanTestExtra is MultiSourceCommons {
    ...
    function testSetDelegationRegistry() public {
        address newDelegate = address(8888);
        vm.prank(_msLoan.owner());
        _msLoan.setDelegateRegistry(newDelegate);
        assertEq(_msLoan.getDelegateRegistry(), newDelegate);
    }
    ...
    function testMergeTranches() public {
        (uint256 loanId, IMultiSourceLoan.Loan memory loan) = _setupMultipleRefi(1);
        uint256 extraPrincipal = 1000;
        uint256 newApr = 1000;
        IMultiSourceLoan.RenegotiationOffer memory offer =
            _getSampleRenegotiationNewTranche(loanId, loan, extraPrincipal, newApr);
        offer.lender = loan.tranche[1].lender;
        testToken.mint(offer.lender, offer.principalAmount);
        vm.prank(offer.lender);
        testToken.approve(address(_msLoan), offer.principalAmount);
        ...
        (, IMultiSourceLoan.Loan memory mergedLoan) = _msLoan.mergeTranches(loanId, loan, 1, 3);

        assertEq(loan.principalAmount, mergedLoan.principalAmount);
        assertEq(mergedLoan.tranche.length, 3);
        IMultiSourceLoan.Tranche memory mergedTranche = mergedLoan.tranche[1];
        assertEq(mergedTranche.floor, mergedLoan.tranche[0].principalAmount);
    }

    function testMergeTranchesFailDiffLender() public {
        (uint256 loanId, IMultiSourceLoan.Loan memory loan) = _setupMultipleRefi(2);
        uint256 extraPrincipal = 1000;
        uint256 newApr = 1000;
        IMultiSourceLoan.RenegotiationOffer memory offer =
        ...
        vm.expectRevert(abi.encodeWithSignature("MismatchError()"));
        _msLoan.mergeTranches(loanId, loan, 0, 2);
    }
