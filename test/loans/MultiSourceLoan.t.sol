...
contract MultiSourceLoanTest is MultiSourceCommons {
    ...
    function testUpdateImprovementMinimum() public {
        MultiSourceLoan testMsLoan = _getOtherMsLoan();
        uint256 changed = 100;
        IBaseLoan.ImprovementMinimum memory newMin = IBaseLoan.ImprovementMinimum(100, 100, changed);
        vm.prank(testMsLoan.owner());
        testMsLoan.updateImprovementMinimum(newMin);

        assertEq(testMsLoan.getImprovementMinimum().principalAmount, changed);
    ...
    function testTransferOwnership() public {
        address otherUser = address(11111);
        ...
        vm.prank(_msLoan.owner());
        vm.expectRevert(abi.encodeWithSignature("TooSoonError()"));
        _msLoan.transferOwnership(otherUser);

        vm.warp(_msLoan.MIN_WAIT_TIME() + 1);
        vm.expectRevert(bytes("UNAUTHORIZED"));
        _msLoan.transferOwnership(otherUser);

        vm.startPrank(_msLoan.owner());
        vm.expectRevert(abi.encodeWithSignature("InvalidInputError()"));
        _msLoan.transferOwnership(address(3843));
    ...
    function testPartialRefinanceLockedTrancheEnd() public {
        ...
        IMultiSourceLoan.RenegotiationOffer memory refiOffer =
            _getSampleRefinancePartial(loanId, trancheIdx, loan, true);
    ...
    function testRefinanceFullInvalidSignature() public {
        ...
        testToken.mint(signer, refiOffer.principalAmount);
        vm.prank(signer);
        testToken.approve(address(_msLoan), refiOffer.principalAmount * 2);
    ...
    function testRefinancePartialNoExtra() public {
        ...
        IMultiSourceLoan.RenegotiationOffer memory refiOffer =
            _getSampleRefinancePartial(loanId, new uint256[](1), loan, false);
    ...
    function testRefinancePartialExtra() public {
        IMultiSourceLoan.LoanOffer[] memory offers = new IMultiSourceLoan.LoanOffer[](2);
        offers[0] = _getSampleOffer(address(collateralCollection), collateralTokenId, _INITIAL_PRINCIPAL);
        offers[1] = _getSampleOffer(address(collateralCollection), collateralTokenId, _INITIAL_PRINCIPAL);
        offers[1].maxSeniorRepayment =
            offers[0].principalAmount + offers[0].principalAmount.getInterest(offers[0].aprBps, offers[0].duration);
        offers[1].principalAmount = offers[0].principalAmount * 2;

        IMultiSourceLoan.LoanExecutionData memory lde = _sampleMultipleOffersLoanExecutionData(offers);
        (uint256 loanId, IMultiSourceLoan.Loan memory loan) = _msLoan.emitLoan(lde);

        uint256 lastTrancheIdx = loan.tranche.length - 1;
        uint256[] memory trancheIdx = new uint256[](1);
        trancheIdx[0] = lastTrancheIdx;
        IMultiSourceLoan.RenegotiationOffer memory refiOffer =
            _getSampleRefinancePartial(loanId, trancheIdx, loan, true);

        uint256 lastTranchePrincipal = loan.tranche[lastTrancheIdx].principalAmount;
        uint256 addedPrincipal = refiOffer.principalAmount - lastTranchePrincipal;
        uint256 currentTs = block.timestamp;
        uint256 delta = 1 days;
        vm.warp(currentTs + delta);
        ...
        assertEq(newLoan.principalAmount, loan.principalAmount + addedPrincipal);
        assertEq(newLoan.tranche[lastTrancheIdx].principalAmount, lastTranchePrincipal + addedPrincipal);
    }
    ...
    function testFailExtraNotLast() public {
        ...
        IMultiSourceLoan.RenegotiationOffer memory refiOffer =
            _getSampleRefinancePartial(loanId, new uint256[](1), loan, true);
