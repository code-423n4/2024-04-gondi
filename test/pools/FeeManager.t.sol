...
contract FeeManagerTest is Test {
    ...
    function testSetFees() public {
        ...
        _feeManager.setPendingFees(newFees);

        vm.prank(_feeManager.owner());
        _feeManager.setPendingFees(newFees);

        IFeeManager.Fees memory pendingFees = _feeManager.getPendingFees();

        assertEq(pendingFees.managementFee, newFees.managementFee);
        assertEq(pendingFees.performanceFee, newFees.performanceFee);
        assertEq(_feeManager.getPendingFeesSetTime(), block.timestamp);
        ...
        assertEq(_feeManager.getPendingFeesSetTime(), type(uint256).max);
    }
