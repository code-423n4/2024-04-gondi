...
contract PoolTest is MultiSourceCommons {
    ...
    function setUp() public override {
        ...
        _poolOfferHandler.confirmTerms(termKeys, terms);
        vm.stopPrank();
        vm.warp(1);
    ...
    function testReallocate() public {
        ...
        uint256 expectedShares =
            toBeReallocated.mulDivDown(_pool.totalSupply() * _pool.getReallocationBonus(), _BPS * _pool.totalAssets());
        _curvePool.setNextDy(toBeReallocated);
        uint256 bonusShares = _pool.reallocate();

        assertEq(bonusShares, expectedShares);
    }
    ...
    function testSetBaseInterestAllocator() public {
        ...
        assertEq(pool.getPendingBaseInterestAllocator(), address(0));
        assertEq(pool.getPendingBaseInterestAllocatorSetTime(), type(uint256).max);
        assertEq(pool.getBaseInterestAllocator(), address(0));

        vm.prank(pool.owner());
        pool.confirmBaseInterestAllocator(address(allocator));

        assertEq(_pool.getBaseInterestAllocator(), address(_baseAllocator));
        assertEq(_asset.allowance(address(_pool), address(_baseAllocator)), type(uint256).max);

        vm.expectRevert(bytes("UNAUTHORIZED"));
        _pool.setBaseInterestAllocator(address(allocator));

        vm.startPrank(_pool.owner());
        _pool.setBaseInterestAllocator(address(allocator));

        assertEq(_pool.getPendingBaseInterestAllocator(), address(allocator));
        assertEq(_pool.getPendingBaseInterestAllocatorSetTime(), block.timestamp);

        vm.expectRevert(abi.encodeWithSignature("TooSoonError()"));
        _pool.confirmBaseInterestAllocator(address(allocator));

        vm.warp(_pool.UPDATE_WAITING_TIME() + 1);

        vm.expectRevert(abi.encodeWithSignature("InvalidInputError()"));
        _pool.confirmBaseInterestAllocator(address(181818));

        _pool.confirmBaseInterestAllocator(address(allocator));

        assertEq(_pool.getBaseInterestAllocator(), address(allocator));
        assertEq(_pool.getPendingBaseInterestAllocatorSetTime(), type(uint256).max);

        vm.stopPrank();
    }

    function testSetUnderwriter() public {
        Pool pool = _getPool();

        address newUnderwriter = address(1111);

        vm.expectRevert(bytes("UNAUTHORIZED"));
        pool.setUnderwriter(newUnderwriter);

        vm.startPrank(pool.owner());
        pool.setUnderwriter(newUnderwriter);

        assertEq(pool.getPendingUnderwriter(), newUnderwriter);

        vm.expectRevert(abi.encodeWithSignature("TooSoonError()"));
        pool.confirmUnderwriter(newUnderwriter);

        vm.warp(pool.UPDATE_WAITING_TIME() + 1);

        vm.expectRevert(abi.encodeWithSignature("InvalidInputError()"));
        pool.confirmUnderwriter(address(2222));

        pool.confirmUnderwriter(newUnderwriter);

        assertEq(pool.getPendingUnderwriterSetTime(), type(uint256).max);
        assertEq(pool.getUnderwriter(), newUnderwriter);
    ...
    function testMint() public {
        ...
        vm.startPrank(_user);
        _asset.approve(address(_pool), tokens);
        uint256 shares = _pool.mint(tokens, _user);
        vm.stopPrank();
    ...
    function testRepaymentWithFees() public {
        ...
        feeManager.setPendingFees(newFees);
    ...
    function testLiquidationWithFees() public {
        ...
        _pool.loanLiquidation(
            1, offerExecution.amount, offerExecution.offer.aprBps, 0, protocolFee.fraction, received, startTs
        );
    ...
    function testLoanRepaymentWithWithdrawalsAndMultipleOutstanding() public {
        ...
        uint256 expectedFromFirstQueue = poolReceived.mulDivDown(shares, _pool.totalSupply());
        uint256 expectedSecondQueue = expectedLiquidSecondQueue + expectedFromFirstQueue;
        ...
        assertApproxEqAbs(
            _asset.balanceOf(address(_pool)), poolBalancePreDeploy - queueClaimable - expectedSecondQueue, 1
        );
        assertApproxEqAbs(_asset.balanceOf(deployedQueue2.contractAddress), expectedSecondQueue, 1);
    ...
    function testLoanLiquidated() public {
        ...
        _pool.loanLiquidation(
            1, offerExecution.amount, offerExecution.offer.aprBps, 0, protocolFee.fraction, proceeds, 1
        );
