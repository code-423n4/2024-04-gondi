...
contract MultiSourceLoanTest is MultiSourceCommons {
    ...
    function testLiquidationGas() public {
        ...
        AuctionLoanLiquidator liquidator = new AuctionLoanLiquidator(
            address(new LiquidationDistributor(address(_loanManagerRegistry))),
        ...
        for (uint256 i = 3; i < sources;) {
            ...
            vm.startPrank(_borrower);
            auction = liquidator.placeBid(address(collateralCollection), collateralTokenId, auction, 1000);
    ...
    function _setupPool() private {
        ...
        pendingCaller[0] = LoanManager.PendingCaller(address(_msLoan), true);
        vm.startPrank(_pool.owner());
        _pool.confirmBaseInterestAllocator(address(_baseInterestAllocator));
