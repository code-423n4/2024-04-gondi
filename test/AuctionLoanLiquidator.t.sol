...
contract AuctionLoanLiquidatorTest is Test {
    ...
    function setUp() public {
        liquidator = new AuctionLoanLiquidator(distributor, whitelistedCurrencies, whitelistedCollections, 0);
    ...
    function testLiquidateLoanSuccess() public {
        ...
        bytes memory encodedAuction =
            liquidator.liquidateLoan(loanId, address(collection), tokenId, address(token), uint96(10), address(this));
    ...
    function testZeroAddress() public {
        ...
        new AuctionLoanLiquidator(address(0), whitelistedCurrencies, whitelistedCollections, 0);

        vm.expectRevert(abi.encodeWithSignature("ZeroAddressError()"));
        new AuctionLoanLiquidator(distributor, address(0), whitelistedCollections, 0);

        vm.expectRevert(abi.encodeWithSignature("ZeroAddressError()"));
        new AuctionLoanLiquidator(distributor, whitelistedCurrencies, address(0), 0);
    }
    ...
    function testPlaceBidAuctionOver() public {
        ...
        token.approve(address(liquidator), 100);
        auction = liquidator.placeBid(address(collection), tokenId, auction, 1);
        ...
        returns (IAuctionLoanLiquidator.Auction memory)
    {
        ...
        vm.prank(loanContract);
        bytes memory encodedAuction =
            liquidator.liquidateLoan(loanId, address(collection), tokenId, address(token), _duration, originator);
        IAuctionLoanLiquidator.Auction memory auction = abi.decode(encodedAuction, (IAuctionLoanLiquidator.Auction));
