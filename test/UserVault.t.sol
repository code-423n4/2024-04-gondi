...
contract UserVaultTest is Test {
    ...
    function setUp() public {
        ...
        vm.startPrank(_collectionManager.owner());
        _collectionManager.add(address(_collection));
        _collectionManager.add(address(_oldCollection));
        vm.stopPrank();

        _userVault = new UserVault(address(_currencyManager), address(_collectionManager));
    ...
    function testBurnAndWithdraw() public {
        ...
        _userVault.burnAndWithdraw(vaultId, collections, tokenIds, tokens);
    ...
    function testBurnAndWithdrawNotApproved() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("NotApprovedError(uint256)", vaultId));
        vm.prank(address(9999));
        _userVault.burnAndWithdraw(vaultId, new address[](0), new uint256[](0), new address[](0));
    }
    ...
    function testDepositVaultNotExists() public {
        ...
        vm.deal(address(this), amount);
        vm.expectRevert(abi.encodeWithSignature("VaultNotExistsError(uint256)", 1));
