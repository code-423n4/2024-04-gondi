...
contract AuctionWithBuyoutLoanLiquidatorTest is MultiSourceCommons {
    ...
    function setUp() public override {
        ...
        liquidator = new AuctionWithBuyoutLoanLiquidator(
            distributor, address(currencyManager), address(collectionManager), 0, 1 days
        );
        ...
        for (uint256 i; i < loan.tranche.length;) {
            if (i != largestIndex) {
                ...
                uint256 thisOwed = principalAmount + loan.tranche[i].accruedInterest
                    + principalAmount.getInterest(loan.tranche[i].aprBps, block.timestamp - loan.tranche[i].startTime);
                owed[i] = thisOwed;
                totalOwed += thisOwed;
            }
            unchecked {
    ...
    function testPlaceBidTooEarly() public {
        ...
        liquidator.placeBid(address(collateralCollection), collateralTokenId, auction, 1000);
    }
